var portfolios=Referential.PortfolioSummaries.Where(i => i.SicavEntityId == Parameters.SicavId).ToList();
...
foreach (var port in portfolios)
{
	...
	var row = NewListRow(port.Name, new []
					...
					NewValue("Sum >5%").SetPercentage(sum)
									.SetSentiment(sum<0.3?MacroResultSentiment.Positive:
														MacroResultSentiment.Negative)
					});
